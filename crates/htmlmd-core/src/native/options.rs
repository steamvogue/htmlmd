...
//! htmd's `Options` struct and enums, ported verbatim so the native renderer
//! does not depend on the `htmd` crate. Field and variant semantics are
//! identical to htmd v0.5.4 `src/options.rs`.
...
pub enum CodeBlockStyle {
    /// Not produced by the `ConversionOptions` mapping yet; preserved for
    /// parity with htmd's options schema (Phase B/C may expose it).
    #[allow(dead_code)]
    Indented,
    Fenced,
}
...
pub enum LinkStyle {
    ...
    /// Not produced by the `ConversionOptions` mapping yet; preserved for
    /// parity with htmd's options schema.
    #[allow(dead_code)]
