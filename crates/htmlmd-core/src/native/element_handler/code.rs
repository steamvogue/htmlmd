...
pub(super) fn code_handler(handlers: &dyn Handlers, element: Element) -> Option<HandlerResult> {
    ...
    if handlers.options().translation_mode == TranslationMode::Faithful
        && !element.node.children().all(|node| node.value().is_text())
    {
    ...
    if handlers.options().code_block_style == CodeBlockStyle::Fenced {
        ...
        let language = find_language_from_attrs(element.attrs).or_else(|| {
            if let Node::Element(parent_element) = parent.value() {
                find_language_from_attrs(&parent_element.attrs)
            } else {
                None
            }
        });
...
fn find_language_from_attrs(attrs: &[(QualName, StrTendril)]) -> Option<String> {
    attrs
        .iter()
